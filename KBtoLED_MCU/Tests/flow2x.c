/*
Test full flow that should be able to handle multiple keypresses
that are relatively soon after each other
*/
...
#define MAX_PRESSES 5 //maximum number of presses to handle within a short time
...
void process_presses(uint8_t LED0[N][3], uint8_t LED[N][3], press presses[MAX_PRESSES], int numpresses);
...
int main(void){
  ...
  key keys[K];
  init_keys(keys); //lookup table for key locations
...
void process_presses(uint8_t LED0[N][3], uint8_t LED[N][3], press presses[MAX_PRESSES], int numpresses){
    ...
    for (i = 0; i < N; i++){
        wave = 0;
        for (p = 0; p < numpresses; p++){
            x = presses[p].loc;
            t = presses[p].t;
            wave += wave_function(i, x, t);
        }
        LED[i][0] = LED0[i][0] - wave*BASE_COL;
        LED[i][1] = LED0[i][1] - wave*BASE_COL;
        LED[i][2] = LED0[i][2] - wave*BASE_COL;
    }
}